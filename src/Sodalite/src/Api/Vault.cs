...
public static class VaultAPI
{
	...
	public static SavedGun SaveGun(FVRFireArm firearm)
	{
		...
		var forward = detectedObjects[0].transform.forward;
		var up = detectedObjects[0].transform.up;
		...
		for (var i = 0; i < detectedObjects.Count; i++)
		{
			...
			if (i > 0)
			{
				var transform = detectedObjects[0].transform;
			...
			switch (detectedObjects[i])
			{
				...
				case FVRFireArm firearmObj:
				{
					...
					if (firearmObj.Magazine != null)
						for (var j = 0; j < firearmObj.Magazine.m_numRounds; j++)
				...
				{
					for (var k = 0; k < firearmClip.m_numRounds; k++) savedGun.LoadedRoundsInMag.Add(firearmClip.LoadedRounds[k].LR_Class);
				...
				{
					foreach (var c in speedloader.Chambers.Where(c => c.IsLoaded)) savedGun.LoadedRoundsInMag.Add(c.LoadedClass);
				...
				case FVRFireArmAttachment attachment:
				{
					...
					var curMount = attachment.curMount;
					var myObject = curMount.MyObject;
	...
	private static IEnumerator SpawnGun_Internal(SavedGun gun, Vector3 position, Quaternion rotation)
	{
		...
		for (var j = 0; j < gun.Components.Count; j++)
		{
			var gameObject = Object.Instantiate(callbackList[j].Result);
			...
			{
				var myMagazine = gameObject.GetComponent<FVRFireArmMagazine>();
				validIndexes.Add(j);
				if (myMagazine != null)
				{
			...
			else
			{
				...
				{
					var component = gameObject.GetComponent<Speedloader>();
				...
				{
					var component2 = gameObject.GetComponent<FVRFireArmClip>();
		...
		var breakIterator = 200;
		while (attachments.Count > 0 && breakIterator > 0)
		{
			breakIterator--;
			for (var k = attachments.Count - 1; k >= 0; k--)
			{
				var savedGunComponent = dicGO[attachments[k]];
				if (validIndexes.Contains(savedGunComponent.ObjectAttachedTo))
				{
					var gameObject2 = attachments[k];
					var component3 = gameObject2.GetComponent<FVRFireArmAttachment>();
					var mount = dicByIndex[savedGunComponent.ObjectAttachedTo].GetComponent<FVRPhysicalObject>()
					...
					if (component3.CanScaleToMount && mount.CanThisRescale()) component3.ScaleToMount(mount);

					component3.AttachToMount(mount, false);
					if (component3 is Suppressor suppressor) suppressor.AutoMountWell();
		...
		for (var l = 0; l < trayObjects.Count; l++)
		{
