...
public static class FirearmAPI
{
	...
	public static List<FVRPhysicalObject> GetAttachedObjects(this FVRFireArm fireArm)
	{
	...
	/// </summary>
	public static List<FVRObject> GetCompatibleMagazines(this FVRObject firearm)
	{
		firearm = IM.OD[firearm.ItemID];
		return firearm.CompatibleMagazines.Concat(firearm.CompatibleClips).Concat(firearm.CompatibleSpeedLoaders).ToList();
	}
	...
	public static FVRObject? GetSmallestMagazine(IList<FVRObject> pool, Func<FVRObject, bool>? filter = null)
	{
		...
		var smallestCap = (filter is null ? pool : pool.Where(filter)).Select(x => x.MagazineCapacity).Min();

		// Select all the magazines with that capacity
		var valid = (filter is null ? pool : pool.Where(filter)).Where(x => x.MagazineCapacity == smallestCap);

		// Return a random item from the array or null if there are none
		var array = valid.ToArray();
