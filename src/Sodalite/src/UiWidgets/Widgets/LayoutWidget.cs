...
public abstract class LayoutWidget<TLayout> : UiWidget where TLayout : LayoutGroup
{
	...
	{
		var widget = CreateAndConfigureWidget(gameObject, configure);
...
public class GridLayoutWidget : LayoutWidget<GridLayoutGroup>
{
	...
	public void SetGrid(int rows, int columns)
	{
		...
		var gridSize = RectTransform.sizeDelta;
		var cellWidth = (gridSize.x - columns * LayoutGroup.spacing.x) / columns;
		var cellHeight = (gridSize.y - rows * LayoutGroup.spacing.y) / rows;
